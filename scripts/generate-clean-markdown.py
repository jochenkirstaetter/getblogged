...
def process_and_copy_markdown(source_dirs, output_dir: Path):
    ...
    for src_dir in source_dirs:
        ...
        for md_file in src_dir.glob('**/*.md'):
            # Skip toc.yml, index files or hidden files
...
def main():
    ...
    site_dir = repo_root / 'posts' / '_site'
    ...
    # In draft mode, copy the generated index-draft.html to index.html in _site output
    if is_draft:
        draft_html = site_dir / 'index-draft.html'
        index_html = site_dir / 'index.html'
        if draft_html.exists():
            shutil.copyfile(draft_html, index_html)
            print(f"Set {index_html} from {draft_html} for draft landing page.")
